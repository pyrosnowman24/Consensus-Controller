...
robot = End(1,[.25,.75],bigS,rospy.get_time(),bounding_box)
...
def initialize():
    ...
    data = rospy.Publisher("/robot2/data",Data,queue_size=1)
    rate = rospy.Rate(20)

    data.publish(robot.pos[0],robot.pos[1],robot.Sensor.sensor(robot.pos))
    
    while not rospy.is_shutdown():
        robot.positions[2,:] = robot.pos
        robot2.updateVoronoi()
        centroid = robot2.computeCentroid()
