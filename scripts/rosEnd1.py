...
robot = End(0,[.25,.25],bigS,rospy.get_time(),bounding_box)
def posesCallback(data):
    robot.pos = data[:,1]
def modelCallback(data):
    robot.p = data
def robot2Callback(data):
    robot.positions[2,:] = [data.x,data.y]
def robot3Callback(data):
    robot.positions[3,:] = [data.x,data.y]
def robot4Callback(data):
    robot.positions[4,:] = [data.x,data.y]
...
def initialize():
    ...
    data = rospy.Publisher("/robot1/data",Data,queue_size=1)
    rate = rospy.Rate(20)

    data.publish(robot.pos[0],robot.pos[1],robot.Sensor.sensor(robot.pos))

    while not rospy.is_shutdown():
        robot.positions[1,:] = robot.pos
        robot.updateVoronoi()
